...
> **Status:** v1.4.0. Every line of the definition of done in [SPEC.md](SPEC.md) has shipped, including tmux `-CC` control mode, profiles, triggers and settings search, and ⌘K now searches every tab and command in one list. Two of the performance targets have been measured rather than asserted: scrollback memory is met with room to spare, throughput is well short of the figure the spec aspired to. Both numbers, and the harness that produced them, are in [docs/BENCHMARKS.md](docs/BENCHMARKS.md).
...
- ⌘-click copies text set apart by colour — inline code or a quoted draft in Claude Code's output, a blue name in `ls`. The run is every neighbouring cell in the same colour, so it starts and stops mid-line; with ⌘ held it tints under the pointer, and ⌘-click asks before copying, showing the text and its character count. A coloured paragraph the program word-wrapped itself copies as one line, but a column of short same-coloured lines stays one line each. Links win where they match.
...
- Clear Screen (⇧⌘K) asks first, because it discards the scrollback along with the screen and sits one key away from the ⌘K hub. Togglable in Settings.
- Font family, size, stroke weight, and line spacing (1.0×–2.0×, default 1.15×) are all adjustable live in Settings.
- Frames can be presented without waiting for the display's refresh (Appearance > Rendering > Sync frames to the display, on by default): a few milliseconds off a keystroke's echo, at the risk of a torn frame.
