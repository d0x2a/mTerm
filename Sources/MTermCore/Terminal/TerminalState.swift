...
package struct CellAttrs: OptionSet {
    package let rawValue: UInt8
    package init(rawValue: UInt8) { self.rawValue = rawValue }
    package static let bold      = CellAttrs(rawValue: 1 << 0)
    package static let italic    = CellAttrs(rawValue: 1 << 1)
    package static let underline = CellAttrs(rawValue: 1 << 2)
    package static let inverse   = CellAttrs(rawValue: 1 << 3)
    package static let faint     = CellAttrs(rawValue: 1 << 4)
}

package struct Cell {
    package var scalar: Unicode.Scalar
    package var fg: PackedColor
    package var bg: PackedColor
    package var attrs: CellAttrs
    ...
    package var link: UInt16

    package init(scalar: Unicode.Scalar = " ",
         fg: PackedColor? = nil,
         bg: PackedColor? = nil,
         attrs: CellAttrs = [],
         width: UInt8 = 1,
...
package struct TerminalSnapshot {
    ...
    package let rowWrapped: [Bool]
    package let cursorCol: Int
    package let cursorRow: Int
    package let cursorVisible: Bool
    package let scrollbackLines: Int       // total lines available in scrollback
    package let scrollOffset: Int          // how many of those are showing above the grid
    package let title: String              // last OSC 0/2 title from the shell
    package let prompts: [PromptMark]      // prompt markers currently visible in the viewport
    package let scrolledRows: Int          // total lines ever pushed to scrollback (for absolute coords)
    package let currentDirectory: String?  // last OSC 7 reported cwd
    package let usingAlt: Bool             // alt buffer is showing (vim, less, ...)
    /// OSC 8 hyperlink targets, indexed by `Cell.link` - 1. Empty until
    /// something on screen actually emits one.
    package let links: [String]

    // Spelled out: a synthesized memberwise init is internal to this module.
    package init(cols: Int,
                 rows: Int,
                 cells: [Cell],
                 rowOffset: Int,
                 rowWrapped: [Bool],
    ...
    @inline(__always)
    package func rowStart(_ row: Int) -> Int {
...
package struct SearchMatch: Equatable {
    package let absoluteLine: Int
...
package struct PromptMark {
    package let viewportRow: Int
...
package struct Selection {
    ...
    package func contains(col: Int, row: Int) -> Bool {
...
package final class TerminalState: ParserSink {
    ...
    /// is not reflowed on.
    package let reflowsOnResize: Bool
    ...
    package var promptAbsoluteLines: [Int] { prompts.map { $0.absoluteLine } }
    ...
    package private(set) var bracketedPaste: Bool = false   // ?2004
    package private(set) var reportFocus: Bool = false      // ?1004
    package private(set) var mouseTracking: MouseTracking = .off
    package private(set) var mouseEncoding: MouseEncoding = .x10
    ...
    package private(set) var alternateScroll: Bool = true   // ?1007
    ...
    package var onReply: (([UInt8]) -> Void)?
    ...
    package private(set) var currentDirectory: String? = nil
    ...
    package var synchronizedUpdateDeadline: CFAbsoluteTime? { syncUpdateDeadline }

    /// True while the child holds a synchronized update open (DEC 2026), until
    /// the safety deadline passes.
    package var synchronizedUpdateActive: Bool {
    ...
    package var onDCSStart: ((_ params: [Int], _ final: UInt8) -> Void)?
    package var onDCSPut: ((ArraySlice<UInt8>) -> Void)?
    package var onDCSEnd: (() -> Void)?

    /// Fired on the parser (session) queue when the child writes BEL (0x07).
    /// Session marshals this to the main thread.
    package var onBell: (() -> Void)?
    ...
    package var onNotify: ((_ title: String, _ body: String) -> Void)?
    ...
    package private(set) var usingAlt: Bool = false
    ...
    /// live one.
    package init(cols: Int, rows: Int,
    ...
    package func resize(cols requestedCols: Int, rows requestedRows: Int) {
    ...
    package func snapshot() -> TerminalSnapshot {
    ...
    package func parserPrint(_ scalar: Unicode.Scalar) {
    ...
    package static func displayWidth(_ scalar: Unicode.Scalar) -> Int {
    ...
    package func parserExecute(_ control: UInt8) {
        switch control {
    ...
    package func parserCSI(_ params: [Int], marker: UInt8?, intermediates: [UInt8], final: UInt8) {
    ...
    package func parserOSC(_ data: [UInt8], terminator: UInt8) {
    ...
    /// sidebar picks it up.
    package func parserWindowName(_ name: [UInt8]) {
        title = String(decoding: name, as: UTF8.self)
    }

    package func parserDCSStart(_ params: [Int], intermediates: [UInt8], final: UInt8) {
        onDCSStart?(params, final)
    }

    package func parserDCSPut(_ bytes: ArraySlice<UInt8>) {
        onDCSPut?(bytes)
    }

    package func parserDCSEnd() {
        onDCSEnd?()
    }

    package func applyThemeChange(from old: Theme, to new: Theme) {
        theme = new
    ...
    package func contentBounds() -> (firstLine: Int, lastLine: Int, lastCol: Int)? {
    ...
    package func jumpToPromptOffset(direction: Int, from currentOffset: Int) -> Int? {
    ...
    package func parserESC(_ final: UInt8, intermediates: [UInt8]) {
...
package enum AnsiPalette {
    ...
    package static func indexed256(_ idx: Int, palette: [SIMD4<Float>]) -> SIMD4<Float> {
