...
package enum MouseReport {
    package enum Kind {
        case press, release, drag, motion
    }

    /// Button numbers as they appear on the wire.
    package static let left = 0
    package static let middle = 1
    package static let right = 2
    /// The "no button" code that bare-motion reports carry.
    package static let none = 3
    package static let wheelUp = 64
    package static let wheelDown = 65
    ...
    package static func bytes(kind: Kind,
                      button: Int,
                      col: Int,
                      row: Int,
                      option: Bool = false,
