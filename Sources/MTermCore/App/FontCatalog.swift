#if canImport(AppKit)
import AppKit
/// The host's font class. Both bridge to `CTFont`, which is all the renderer
/// ever wanted — see the `let font: CTFont = ...` in `Renderer.init`.
...
/// The two font lookups that differ between the platforms. Deliberately not
/// unified onto CoreText: `monospacedSystemFont` is SF Mono, and the nearest
/// CoreText equivalent (`.userFixedPitch`) is Menlo — a silent change of the
/// default face. `isFixedPitch` likewise doesn't agree with CoreText's
/// `.traitMonoSpace` on every family, and the picker's contents are a visible
/// thing to change by accident.
extension PlatformFont {
    package static func monospacedSystem(ofSize size: CGFloat) -> PlatformFont {
        #if canImport(AppKit)
    ...
    package static var installedFamilies: [String] {
        #if canImport(AppKit)
    ...
    package static func isFixedPitch(family: String) -> Bool {
        #if canImport(AppKit)
        return NSFont(name: family, size: 12)?.isFixedPitch == true
        #else
        guard let f = UIFont(name: family, size: 12) else { return false }
        return CTFontGetSymbolicTraits(f).contains(.traitMonoSpace)
        #endif
    }
}
...
package enum FontCatalog {
    package struct Entry: Equatable {
        package let displayName: String
        package let postScriptName: String   // ignored for SF Mono
    }

    package static let defaultFamily = "SF Mono"

    package static let curated: [Entry] = [
    ...
    package static var discovered: [Entry] {
        let known = Set(curated.map(\.displayName))
        return PlatformFont.installedFamilies
            .filter { !known.contains($0) && PlatformFont.isFixedPitch(family: $0) }
            .sorted()
    ...
    package static var available: [Entry] {
        curated.filter { entry in
            if entry.displayName == defaultFamily { return true }
            return PlatformFont(name: entry.postScriptName, size: 12) != nil
        }
    }

    /// Builds a font sized for the renderer (pixel-space: `pointSize * scale`).
    /// Falls back to SF Mono if the configured family isn't available.
    package static func makeFont(family: String, size: Double, scale: CGFloat) -> PlatformFont {
        let pixelSize = CGFloat(size) * scale
        if family == defaultFamily {
            return PlatformFont.monospacedSystem(ofSize: pixelSize)
        }
        if let entry = curated.first(where: { $0.displayName == family }),
           !entry.postScriptName.isEmpty,
           let f = PlatformFont(name: entry.postScriptName, size: pixelSize) {
            return f
        }
        ...
        if let f = PlatformFont(name: family, size: pixelSize) {
            return f
        }
        return PlatformFont.monospacedSystem(ofSize: pixelSize)
    }

    package static let minSize: Double = 9
    package static let maxSize: Double = 28
}
...
package final class FontCatalogStore: ObservableObject {
    package static let shared = FontCatalogStore()

    @Published package private(set) var available: [FontCatalog.Entry] = FontCatalog.available
    /// Installed monospaced fonts outside the curated list.
    @Published package private(set) var others: [FontCatalog.Entry] = FontCatalog.discovered

    private init() {}

    /// Re-reads both halves of the list: a handful of font lookups
