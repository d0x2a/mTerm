...
package struct Profile: Identifiable, Codable, Equatable {
    package let id: UUID
    ...
    package var isPlainLoginShell: Bool {
    ...
    /// then home.
    package func startDirectory(override: String? = nil) -> String {
    ...
    package func launchSpec(cwd override: String? = nil) -> LaunchSpec {
...
package struct LaunchSpec {
    ...
    package static func resolveExecutable(_ name: String) -> String {
...
package enum ShellWords {
    package static func split(_ line: String) -> [String] {
        var words: [String] = []
        var current = ""
        var inWord = false
...
package final class ProfileStore: ObservableObject {
    package static let shared = ProfileStore()

    @Published package private(set) var profiles: [Profile] = []
    ...
    package var defaultProfile: Profile {
    ...
    package func profile(id: UUID) -> Profile? {
        profiles.first { $0.id == id }
    }

    package func setDefault(_ id: UUID) {
        ThemeStore.shared.settings.defaultProfileId = id.uuidString
        objectWillChange.send()
    }

    @discardableResult
    package func add() -> Profile {
    ...
    package func update(_ profile: Profile) {
    ...
    package func move(fromOffsets source: IndexSet, toOffset destination: Int) {
