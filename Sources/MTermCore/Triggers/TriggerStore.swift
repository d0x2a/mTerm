...
package final class TriggerStore: ObservableObject {
    package static let shared = TriggerStore()

    /// The user's own triggers, in evaluation order.
    @Published package private(set) var userTriggers: [Trigger] = []
    /// Builtins the user has switched off.
    @Published package private(set) var disabledBuiltins: Set<UUID> = []
    ...
    /// an array of regexes.
    @Published package private(set) var generation: Int = 0
    ...
    package func update(_ trigger: Trigger) {
    ...
    package func setBuiltin(_ id: UUID, enabled: Bool) {
        guard Trigger.builtinIDs.contains(id) else { return }
    ...
    package func isEnabled(_ trigger: Trigger) -> Bool {
    ...
    package static func patternError(_ pattern: String) -> String? {
