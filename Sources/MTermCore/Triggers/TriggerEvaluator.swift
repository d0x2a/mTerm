...
package struct TriggerMatch {
    ...
    package let viewportCol: Int           // first cell
    package let viewportRow: Int
    package let length: Int                // number of cells
    package let text: String               // matched substring
    ...
    package let hyperlink: String?

    package init(id: Int, trigger: Trigger, viewportCol: Int, viewportRow: Int,
...
package final class TriggerEvaluator {
    private var compiled: [(Trigger, NSRegularExpression)] = []

    package var triggers: [Trigger] = [] {
        didSet { recompile() }
    }

    package init(triggers: [Trigger] = Trigger.builtins) {
    ...
    package func evaluate(snapshot: TerminalSnapshot) -> [TriggerMatch] {
