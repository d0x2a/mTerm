...
package protocol TmuxCommandSink: AnyObject {
...
package protocol TmuxPaneSink: AnyObject {
    func receive(_ bytes: [UInt8])
    func transportClosed()
}

package protocol TmuxControllerHost: AnyObject {
...
package final class TmuxController {
    ...
    package init(host: TmuxControllerHost, commands: TmuxCommandSink) {
        self.host = host
    ...
    package func start(cols: Int, rows: Int) {
        isActive = true
        clientCols = cols
        clientRows = rows
    ...
    package func handle(_ event: TmuxEvent) {
    ...
    package func newWindow() {
        send("new-window")
    }

    package func killWindow(id: String) {
        send("kill-window -t \(id)")
    }

    package func selectWindow(id: String) {
    ...
    package func sendKeys(window id: String, bytes: [UInt8]) {
    ...
    package func setClientSize(cols: Int, rows: Int) {
    ...
    package func hasWindow(_ id: String) -> Bool { windows[id] != nil }
}
