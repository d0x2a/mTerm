...
package struct HighlightBand {
    package let col: Int
    package let row: Int       // viewport row
    package let length: Int    // in cells
    package let color: SIMD4<Float>
    package let style: HighlightStyle

    // Spelled out: a synthesized memberwise init is internal to this module.
    package init(col: Int,
                 row: Int,
                 length: Int,
                 color: SIMD4<Float>,
...
package struct GridLayout {
    package let cellWidth: Float       // drawable pixels
    package let cellHeight: Float
    package let ascent: Float          // baseline offset from the top of the cell
    ...
    package let glyphBoxHeight: Float
    /// Blank pixels inserted above the glyph box (half of the extra leading).
    package let glyphBoxTop: Float
    package let margin: Float          // minimum padding around the grid
    package let scale: Float

    package func gridSize(viewportPixels: SIMD2<Float>) -> (cols: Int, rows: Int) {
    ...
    package var origin: SIMD2<Float> { SIMD2<Float>(margin, margin) }
}
...
package final class Renderer {
    ...
    package init(device: MTLDevice, pixelFormat: MTLPixelFormat, scale: CGFloat,
