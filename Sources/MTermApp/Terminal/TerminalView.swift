...
final class TerminalView: NSView, CALayerDelegate {
    ...
    /// AppKit's virtual key codes mapped to the platform-neutral chord the
    /// encoder takes. *Which* key was pressed is AppKit's business — the codes
    /// are its own and UIKit's don't agree — but what bytes it becomes is
    /// `KeyEncoder`'s, shared with every other host.
    private func bytesForKey(_ event: NSEvent) -> [UInt8] {
        ...
        let special: KeyChord.Special?
        switch event.keyCode {
        case 36, 76: special = .enter            // Return / numpad enter
        case 51:     special = .backspace
        case 117:    special = .forwardDelete    // Fn+Backspace
        case 48:     special = .tab
        case 53:     special = .escape
        case 126:    special = .up
        case 125:    special = .down
        case 124:    special = .right
        case 123:    special = .left
        case 115:    special = .home             // Fn+Left
        case 119:    special = .end              // Fn+Right
        case 116:    special = .pageUp
        case 121:    special = .pageDown
        default:     special = nil
        }

        return KeyEncoder.bytes(for: KeyChord(
            special: special,
